...
| `SearchOfferCaptureOptions` | `src/session/search-capture.ts` | `page: Page;`<br>`/** Accept only responses whose mtop `params.method` is this value (or one of these values). */`<br>`requireMethod?: string \| readonly string[];`<br>`requireSortType?: string;`<br>`allowUnscopedWirelessRecommend?: boolean;`<br>`targetPage?: () => number;`<br>`keep?: 'first' \| 'largest';` |
